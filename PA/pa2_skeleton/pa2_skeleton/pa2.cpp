												 ...
												 int ty, int x, int y, char &nextChar)
{
	...
	// empty space in big quad1 (x,y) -> small quad3 (x,y), e.g. 8x8, (0,0) -> 4x4, (0,0)
	// empty space in big quad2 (x,y) -> small quad4 (x-tx,y) e.g. 8x8, (4,0) -> 4x4, (0,4)
	// empty space in big quad3 (x,y) -> small quad1 (x-tx,y-ty) e.g. 8x8, (4,4) -> 4x4, (0,0)
	// empty space in big quad4 (x,y) -> small quad2 (x,y-ty) e.g. 8x8, (0,4) -> 4x4, (0,0)
	quad = locateQuadrant(width, (quad == 1 || quad == 4 ? x : x - tx), (quad == 1 || quad == 2 ? y : y - ty));

	// fill the squares by recursion
	fillPuzzleRecursive(2, puzzleMap, tx + width / 2 - 1, ty + width / 2 - 1, (quad == 1 || quad == 4 ? 0 : 1), (quad == 1 || quad == 2 ? 0 : 1), nextChar); // centre
